...
class Program
{
    static void Main(string[] args)
    {
        ...
        while (true)// Main chat loop
        {
            ...
            string input = Console.ReadLine()?.ToLower()?.Trim();// Reads user input and normalizes it
            ...
            if (input == "exit" || input == "quit") // Checks for exit commands
{
    ...
    static void PlayGreetingAudio()// Plays the greeting audio file
    {
        ...
        try 
        {
            ...
            if (Environment.OSVersion.Platform == PlatformID.Win32NT)// Checks for Windows platform
            {
                // Windows
            ...
            else if (Environment.OSVersion.Platform == PlatformID.Unix)// Checks for Unix-like platforms (Linux, macOS)
            {
                // Linux and macOS
                psi = new ProcessStartInfo
                {
            ...
            else if (Environment.OSVersion.Platform == PlatformID.Unix && Environment.OSVersion.VersionString.Contains("Linux"))
            {
                // Linux
                psi = new ProcessStartInfo
                {
            ...
            else if (Environment.OSVersion.Platform == PlatformID.MacOSX)// Checks for macOS platform
            {
                // macOS
                psi = new ProcessStartInfo
                {
        ...
        {
            Console.WriteLine("⚠️  Bot: Audio greeting could not be played. The audio feature may not be supported on your device.");
        }// Handles any exceptions that occur while trying to play the audio
    }
